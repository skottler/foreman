class CreateInterfaces < ActiveRecord::Migration
  def self.up
    ...
    add_index :interfaces, [:type], :name => 'index_by_type'
    add_index :interfaces, [:host_id], :name => 'index_by_host'
    add_index :interfaces, [:type, :id], :name => 'index_by_type_and_id'

    Host.where(["sp_mac <> ? and sp_ip <> ?", "", ""]).each do |host|
      begin
        sp_ip  = host.read_attribute(:sp_ip)
        sp_mac = host.read_attribute(:sp_mac)
          Foreman::Model::BMC.create! :host_id => host.id, :mac => sp_mac, :ip => sp_ip, :subnet_id => host.read_attribute(:sp_subnet_id),
            :name => host.read_attribute(:sp_name), :priority => 1
          say "created BMC interface for #{host}"
      rescue => e
        say "failed to import interfaces for #{host} : #{e}"
      end
    end
  ...
  def self.down
    ...
    Foreman::Model::BMC.all.each do |bmc|
      if bmc.host_id
