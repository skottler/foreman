class LookupKey < ActiveRecord::Base
  ...
  validates_uniqueness_of :key, :scope => :puppetclass_id, :if => Proc.new { |lookup_key| lookup_key.puppetclass_id && lookup_key.is_param } # unique parameter name per puppetclass
  validates_uniqueness_of :key, :if => Proc.new { |lookup_key| lookup_key.puppetclass_id && !lookup_key.is_param }
  ...
  def apply_default_value_stuff
    # Enforce no_default_value (was deferred to permit order independent assignment to default_value and no_default_value)
